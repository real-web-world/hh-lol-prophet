...
type (
	...
	GameInfo struct {
		...
		Participants []struct { // 参与者详细信息
			ChampionId                Champion `json:"championId"` // 英雄id
			HighestAchievedSeasonTier string   `json:"highestAchievedSeasonTier"`
			ParticipantId             int      `json:"participantId"`
			Spell1Id                  Spell    `json:"spell1Id"` // 召唤师技能1
			Spell2Id                  Spell    `json:"spell2Id"` // 召唤师技能2
			Stats                     struct {
		...
		PlatformId string        `json:"platformId"` // 平台id
		QueueId    GameQueueID   `json:"queueId"`    // 队列id
	...
	GameFolwSessionTeamUser struct {
		...
		TeamId                  TeamIDStr   `json:"teamId,omitempty"`
	}
